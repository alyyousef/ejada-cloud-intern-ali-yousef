...
- **The actual, final documentation:** a pre-existing, separately-maintained LaTeX
  ...
  A compiled copy, `weekTwoDocumentation.pdf`, was later placed directly in
  `C:\Users\yinya\Documents\oci-terraform-lab\weekTwo\` — check that file for the actual final
  state the user ended up submitting, since it may include further edits made after this
  session's changes.
...
on `oci-terraform-lab` showed a clean working tree, up to date with `origin/main`, as of the
...
4. **Confirm which documentation PDF is truly final** — `Research_Thesis.pdf` from
   `documentationW2/` vs. `weekTwoDocumentation.pdf` sitting in the main repo folder — and
   confirm where/how it actually gets delivered to the team (its source repo is separate from
   the main one).
...
- A compiled PDF copy also sits at: `C:\Users\yinya\Documents\oci-terraform-lab\weekTwo\weekTwoDocumentation.pdf`
